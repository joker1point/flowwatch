...
- ⚠️ **净收益测量（2026-09-17，`scripts/etw_benefit_measure.py`）—— 结论：测不出净收益，别当它已证明有用**
  ...
  **所以现在的诚实结论是**：
  ...
  - 与全部观测同时吻合的一个**未证实假设**：**这台机器在当 NAT 网关**（`SharedAccess`/ICS 服务在运行）。
    ICS 转发的流量会以宿主地址出现、**没有宿主 socket**、也**没有带 PID 的 ETW 连接事件** ——
    与"165 ms 高频采样也查不到端口""ETW 学到的键无同端口候选""98% 是 TCP"三条同时吻合。
    要证实需要抓全部接口或查 ICS 配置（需管理员），这是下一步。
