...
只是没有采集数据 —— 页头会显示「未启用采集」，页面上出现一块**首启引导**：说清缺的是 Npcap、给出
三步（去官网装 / 不用重启电脑或程序 / 回来点按钮）与下载链接，并带一个「我已装好，重新检测」按钮。
**装完驱动不必重启程序**：该按钮走 `POST /api/capture/retry`，会在进程内重新加载 `wpcap.dll` 并开始采集
（懒加载属性不缓存异常，所以重试真正可行）；没有驱动时它只会如实回 `ok=false`，绝不会把服务弄死。

`test_no_npcap.py` 把这些契约钉进了 CI：页面照开、重试如实失败且进程存活。
...
| 抓包依赖 | 需装 Npcap（管理员 + 驱动） | 驱动**无法内置**（内核驱动，不随包分发）；页面上给**首启引导**（三步 + 下载 + 「重新检测」免重启，2026-09-23 落地）。候选（未拍板）：**纯 ETW 降级档** —— 系统自带的 `Microsoft-Windows-Kernel-Network`（进程 + 远端地址 + 字节数）+ `Microsoft-Windows-DNS-Client`（域名→IP）不依赖任何第三方驱动，代价是精度与域名覆盖退化、且要重排启动顺序（`pick_devices()` 目前在 `etw.start()` 之前） |
...
| 同网段的邻居流量 | 两端都不是本机地址（广播域里的邻居帧） | 不计入本机统计（`foreign_packets`） |
| 非 TCP/UDP | ARP / ICMP / 头部不全 | 不参与统计（`skipped_packets`） |

### 三段实测，每段都有对照

1. **混杂模式会顺手抓到同网段邻居的流量**。第一版 `pcap_open_live(..., promisc=1)`，抓到了
...
- **哨兵桶**：`-1` 属主受限 / `-2` 未归因 / `-3` 同网段的邻居流量 / `-4` 非 TCP-UDP ——
  整机曲线能对账（连"邻居流量"都单独成桶），进程排行只取 `pid > 0`。
...
## 抓包看不见的那一层（一次 207 GB 的真实事故，与由此长出的「本机连接」）

**事故**（2026-09-18）：代理服务商通知这台机器连续四天大量上传，合计约 **207 GB**，
下载不足 4 GB —— 上游只给了总量，没给"是谁"。

**手工取证**：翻 196 MB / **138 万行**的服务日志、查任务库、再用
`Get-NetTCPConnection -RemotePort 7890 | Group-Object OwningProcess` 按进程数连接 ——
**292 条 vs 第二名 3 条**，一个本地后端服务的"图床上传重试风暴"被一把锁定：
单张 1.45 MB 的图片，在全部上传通道失效后陷入"失败 → 退避 → 重试"闭环，
单任务累计尝试 **43052 次**，日志里 **50 万行**上传失败（完整取证报告另存，未入库）。

**然后发现工具失明** —— 当晚打开 flowwatch 想复核流量构成，面板上是这样的：

- 流量榜首是**代理内核**（113 GiB），真正的问题进程**根本不在榜上**；
- 域名榜前几名全是**代理节点域名**，真实目标一个也看不见。

两个盲区，都实测过：

| 盲区 | 实测 | 原因 |
|---|---|---|
| 纯环回不经过网卡 | `/api/rates` 里 `127.0.0.1` 出现 **0** 次 | 「应用 → 本地代理（127.0.0.1:7890）」这一段是环回，NPF 抓不到 |
| 代理同时替换两端 | 进程全归代理内核、域名只剩节点域 | 真实发起方被吞进隧道，真实目标被 TLS 加密 |

这不是 bug，是**抓包视角的边界**：它擅长回答"网线上跑了什么"，
但代理之后的流量在网线上**只剩代理自己的身影** —— "谁在使用网线"这一层，
只能从系统连接表补回来。

**由此长出（本版本新增）**：把那次手工取证的核心一步自动化 ——
系统连接表快照（`psutil.net_connections`，1.5s 刷新，实测 ~20 ms），按进程聚合
**连接总数 / 指向本机的连接数 / 新出现的本机侧端点速率**，外加一张
「本机监听端口 ← 哪些进程在连它」的服务表（见 `localconn.py`）。

界面上它是右列新增的「**本机连接**」面板，进程排行里带 `本机 N 连` 徽标。
**下一次同类事故的画面**：打开面板 → 连接数排第一的进程 → 点开看明细 ——
那次要手敲命令、翻百万行日志才拿到的答案，现在是一眼。

> 一句话：**抓包看得见"网线上跑了什么"，看不见"谁在使用网线"；
> 代理场景把后者藏得最深，而它恰好是排障的第一个问题。**
...
**历史与分析**（1 小时 / 6 小时 / 24 小时 / 7 天 / 30 天）—— 分层曲线（本机已归因 / 属主受限 /
未归因 / 同网段的邻居流量分开画）+ 区间进程榜与域名榜：

![历史与分析](docs/demo-history.gif)

**流量助手**：问「GameViewerServer.exe 是哪个软件？」—— 面板里真敲真答（工具调用行 → 结论）：

![流量助手动图](docs/demo.gif)

> **完整演示（约 2 分钟）**：进程视图 → 点进程看明细与历史 → 域名视图 → 历史与分析（7 天 / 30 天切换）
> → 笔记区（用户笔记 + AI 每日笔记 / 人设）→ 助手四问（排行 / 心跳判断 / 进程身份 / 事件）
> → 模型设置（明细档才给对端）。[**B站 · BV14ehv6REqU**](https://www.bilibili.com/video/BV14ehv6REqU/)｜本机真实运行录制，无摆拍。
> 有问题或想法**欢迎开 Issue / PR**。
...
| 状态 | `get_health` |
| 身份 | `get_process_identity`（读 exe 版本资源，不靠名字猜） |
| 记忆 | `memory_insert`、`memory_replace`、`memory_rethink`、`conversation_search` |

分档：取数 8 个（7 个聚合档 + `get_live_connections` 一个明细档）+ 记忆 4 个；
清单与分级规则可以直接问后端 —— `GET /api/assistant/status` 会把 `tools`（12 条）与 `routing` 一起吐出来。

没有任何写监控状态的工具 —— Agent 不能停采集、不能改配置，对外也不发请求。
措辞上要说准：取数 8 个**全部只读**；记忆 4 个里 **3 个会写**，写的是助手自己的记忆库
（`assistant_memory.db`），另一个（`conversation_search`）只读回溯。
...
助手最危险的失败不是答错，而是**没查却看起来像查过**（09-20 真实事故：一轮编出"我查了实时帧与排行，
都没有"的整段结论）。现在的防线是四层，每一层都有回归用例（`evals/`）：

| 层 | 做法 | 生效时机 |
|---|---|---|
| 提示词 | 数据问题必须先取证、不许描述没做过的动作、点名先检索 | 每轮 |
| **协议层强制** | 纠正轮向 provider 声明 `tool_choice="required"`（不认这个字段的 provider 自动退回 `auto`，不报错） | 首轮零工具调用 |
| **系统侧补查** | 模型两次都没调工具 → 服务端按问题类型跑确定性取数（点名对象走 `match` 检索；明细档再补一步"排行定位 pid → 连线明细"），把结果作为内部信息注回上下文让它重答 | 整轮一个工具都没调用 |
| 如实标注 | 连补查也取不到数据（**空结果不算证据**）→ 答复加 `⚠️ 未核实` 前缀；`done` 事件带 `grounded` / `retried` / `fallback` | 无证据时 |

补查代跑的工具在面板上带 `origin=system`，并在 `done.fallback` 里如实列出 —— 系统可以兜底，
但不冒充是模型查的。**红线没有松动**："调过工具却没拿到数据"（参数非法、只调了记忆工具）不走补查，
继续由"未核实"标注兜底。

### 只读承诺：写不了，而不是"我们不写"

助手能做的事只有两件：**读监控数据、写自己的记忆库**。这条边界用三层证明，不靠文档自觉：

- **静态**（`tests/test_readonly.py` 的 §1~4，进 CI）：从每个工具出发做**调用图分析** —— 工具可达的
  代码里没有写文件 / 起进程 / 开网络 / 写历史库；工具名不含写动词；两档工具并集等于完整清单。
- **运行时**（同一个文件的 §5）：装 `sys.addaudithook`，真跑一遍**全部 12 个工具 + 一轮对话**，
  断言真实副作用只有自己的记忆库（含 `-wal`/`-shm`）、**零出站连接**、零子进程、零删除/重命名，
  并带两条**阳性对照**（故意写文件、把副作用藏进读函数内部）证明"真出副作用会被抓到"。
- **结构**：历史库以 `ReadOnlyStore` **只读门面**注入 —— `append` / `submit` / `stop_writer`
  这些写方法在助手拿到的对象上**根本不存在**（与"明细工具在聚合档不注入"同一套路）。执行前还有
  两道硬闸：明细工具在非明细档直接拒、`FLOWWATCH_ASSISTANT_MEMORY_WRITE=off` 时写记忆直接拒
  —— 模型凭名字幻觉调用也没用（**它只是"没被提供"，不代表"调不动"，所以要有闸**）。
- **注入用例**：评估集里有诱导与越权用例（诱导调明细工具、往记忆里塞指令、关掉开关后硬调
  `memory_insert`），并由变异测试自证"把任何一层拆掉，用例都会变红"。

### 权限模型：现在有什么、没有什么（2026-09-23 盘点，本轮收尾）

`permission` 这类系统一般由五块组成。逐块对照，避免以后把"够用"误当成"做完了"：

| 要素 | 成熟实现（如 Claude Code） | 本项目现状 |
|---|---|---|
| 判定种类 | allow / deny / **ask**（人机回路）/ passthrough | **allow / deny** —— **没有 ask** |
| 权限模式 | bypass / acceptEdits / auto 等多档 | 1 个：`FLOWWATCH_ASSISTANT_MEMORY_WRITE`（`off` = 结构性关掉写记忆） |
| 检查点位置 | 多阶段管线 + 固定规则顺序 | **两处**：注入面（`tools_for`）+ 执行前硬闸（`run_tool`），后者不依赖前者 |
| 策略来源 | 多层 settings 合并 + 优先级 | **1 个来源**（环境变量） |
| "不可绕过"的保证 | 内容相关 ask 排在 `bypassPermissions` **之前**（靠顺序） | **复合保证测试**：注入面放宽 / 幻觉调用都过不了执行闸 —— 本路线不依赖"检查点没被绕过" |

**可以宣布收尾**：就当前定位（单机、单用户、只读、12 个工具），"只读"这条承诺已被三层证明 + 变异测试
钉死，且没有已知绕过路径（没有 hook 的 allow 档、没有 bypass 模式、没有多层规则合并的歧义）。
另一条值得说明的差异：本项目**不靠内容黑名单**（字符串匹配挡不住命令变体，成熟实现自己也只是"教学示意"），
走的是**能力不可得** —— 工具不注入、写方法在对象上不存在、执行前硬闸。

**明确没有的三件事（各有触发条件，到点才补）**：

| 缺口 | 何时必须补 | 为什么现在不补 |
|---|---|---|
| **ask 档**（危险操作要人点一下） | 出现能**不可逆改写用户环境**的工具（导出 / 改配置 / 清数据） | 现有唯一写面是助手自己的记忆库：可清空重建、无破坏性，deny 就够 |
| **可配置策略表**（allow/deny 规则 + 优先级） | 多用户 / 企业策略（"运营只能问聚合、SRE 能问明细"） | 单机单用户，多一层合并只会多一处排障点 |
| **参数级语义检查**（例："这条查询是否只读"） | 新增 `run_query` 这类"同一工具既能读也能写"的能力 | 现有工具的参数只决定"查什么范围"，不决定"能不能写"；参数只做类型/范围校验 |

这三条都挂在 `docs/SPEC-assistant-roadmap.md` §3 的 Hook 注册表上（它是权限策略的载体），
且**前置条件已满足**：不变式测试（`tests/test_readonly.py` 的"复合保证"）已经绿了，
真要补时不必先补前置。
...
| `FLOWWATCH_ASSISTANT_MEMORY_WRITE` | `auto`（默认，助手可写**自己的**记忆库）/ `off`（结构性关闭：写记忆的工具不再注入给模型，执行前也会被拒）—— 想要"无状态助手"就设它 |
...
- **告警规则可配置**：代理哨兵（`scripts/proxy_sentinel.py`）目前阈值与"盯哪个进程"都是写死的
  （`%flclash% / %mihomo% / %clash%`，30 分钟 5 GB / 60 分钟 10 GB）—— 换成别的代理就不会触发。
  计划把阈值、窗口、进程匹配都做成配置（CLI + 配置文件）。
- **邮件告警通道**：现有三条出口是桌面弹窗 / Catrace 小窗 / 桌面告警文件；计划再加一条邮件
  （标准库 `smtplib`，零新依赖；凭据进 `.env`，不入库，推不到就降级回弹窗）。

**明确不做**

- **流量封控 / 阻断**：flowwatch 只**告警**、不**处置** —— 不碰网络栈，不改路由、不断连接、不封端口，
  判断权与处置权都在你手上。原因有二：误伤代价高（你可能正在上传大文件，或代理正在正常替别的
  程序转发）；以及那会把"观测工具"变成"处置工具"，责任面完全不同。
