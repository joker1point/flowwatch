...
- ⚠️ **净收益测量（2026-09-17，`scripts/etw_benefit_measure.py`）—— 结论：测不出净收益，别当它已证明有用**
  ...
  **所以现在的诚实结论是**：
  ...
  - 因此 ETW 当前的定位应该是：**机制已验证可用、净收益未证实**。留着它的价值在"可验证的负结果 +
    ...
    下一步真正该查的是残余缺口本身：那些流的本地端口**从不出现在任何 socket 表里**
    （与早期 `diag_miss.py` 的 79.5% "表里从未出现过"一致）→ 最可能是**本机转发的流量**
    （虚拟机 / WSL / 容器 / 网络共享的 NAT 流量：宿主上没有对应的 socket，
    任何基于"连接表 + PID"的方法都归不了它）—— 这在文档里应该如实说明，而不是算作"未归因缺陷"。
