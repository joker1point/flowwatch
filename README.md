...
- ⚠️ **净收益测量（2026-09-17，`scripts/etw_benefit_measure.py`）—— 结论：测不出净收益，别当它已证明有用**
  ...
  **所以现在的诚实结论是**：
  ...
  - **"这台机器在当 NAT 网关"这条假设已被证伪**（2026-09-17 提权实测，`scripts/ics_probe.py`）：
    ...
    而不是把自洽的猜测当结论写进文档。
  - **于是残余缺口的成因目前未定**。已排除的：UDP/QUIC（只占 0.5% 字节）、本机转发/NAT（已证伪）、
    ETW 键归一化错误（近邻诊断排除）。剩下的真问题是：
    **这些 TCP 流的本地端口为什么从不出现在连接表里（165 ms 高频采样也是）**。
    要回答它需要对**单条流**做深挖（抓到那一瞬间：TCP 标志位、连接状态、是否重传/延迟报文），
    而不是继续猜 —— 这条已如实标注为**开放问题**。
