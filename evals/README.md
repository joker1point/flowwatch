...
## 已知口径问题（评估集自己发现并记下来的，不是 bug 隐藏）

1. **参数校验失败的取数调用仍被算成证据**（`args-invalid-01`）：`grounded` 只看"调没调 `get_*`"，
   没看它是否成功返回数据 —— 需要修的话，判据应改为"成功返回数据的取数工具才算证据"。
2. **纯记忆请求也会被打"未核实"前缀**（`memory-write-01`）：问题里点了 `.exe` 就命中数据意图，
   而记忆工具不算证据 —— 于是"帮我记一下"的收据会被标 ⚠️。是否放宽待定。
